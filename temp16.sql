...
WITH year_2013 AS(
  SELECT EXTRACT(YEAR FROM dt) as year, EXTRACT(MONTH FROM dt) AS month, AVG(avg_temp) as avg_temp_2013
...
year_2012 AS(
  SELECT EXTRACT(YEAR FROM dt) as year, EXTRACT(MONTH FROM dt) AS month, avg(avg_temp) as avg_temp_2012
FROM temperatures_by_country
